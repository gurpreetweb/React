...
class Conference extends Component {
  ...
  componentDidMount() {
    ...
    // AGENDA_APIS.createAgendaJoin(agenda_id, userData._id);
  }
  ...
  handleOnConnect = (data) => {
    ...
    AGENDA_APIS.AddSponsorparticipantCount(agenda_id, "increment").then((result) => {
  ...
  handleOnLeave = () => {
    ...
    const { match } = this.props;
    const agenda_id = match.params.conferenceId;
    AGENDA_APIS.AddSponsorparticipantCount(agenda_id, "decrement").then((result) => {
      if (result.status == 200) {
  ...
  render() {
    const { match } = this.props;
    const userData = JSON.parse(localStorage.getItem("userData"));
    let Full_name = (userData.role==4)?userData.sponsor_name:userData.first_name + " " + userData.last_name;
    const userInfo = {
      name:
