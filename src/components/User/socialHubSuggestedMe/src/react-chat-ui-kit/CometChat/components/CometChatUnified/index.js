...
class CometChatUnified extends React.Component {
  ...
  state = {
    ...
    messageScreen:"Select a chat to start messaging"
  };
  ...
  async componentDidMount() {
    ...
    var config = {
      method: "post",
      url: "https://admin.illumeetvirtual.com/api/fetchUserHaveSameInterest",
      headers: {
        token:
    ...
    axios(config)
      .then(function (response) {
        console.log("===================resspone data a2-----------",response.data);
        if(response.data.status==401){
          that.setState({
            messageScreen:"Please complete your profile to communicate with others."
          },()=>{
            console.log("this.state a2=============",that.state)
          })
        }
        else if(response.data.status==200 && response.data.data.length==0){
          that.setState({
            messageScreen:"You have a unqiue interest causing not to found other person have same interest."
          },()=>{
            console.log("this.state a2=============",that.state)
          })
        }
        else{
          console.log("a2==========================",that.state)
        }
      })
  ...
  render() {
    ...
    let messageScreen = <h1>{this.state.messageScreen}</h1>;
    if (Object.keys(this.state.item).length) {
      messageScreen = (
