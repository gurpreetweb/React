...
class CometChatUnified extends React.Component {
  ...
  state = {
    ...
    item: {
      hasBlockedMe: false,
      blockedByMe: false,
      uid: "5f0eec135baa9472eaf8b24f",
      name: "Admin",
      avatar: "https://admin.illumeetvirtual.com/uploads/1597128201820-grid-5.png",
    },
    type: "user",
    tab: "contacts",
    groupToDelete: {},
  ...
  async componentDidMount() {
    const admimdataRes1 = await axios.get(
      `https://admin.illumeetvirtual.com/api/fetch/adminData`,
      { headers: { token: "ashgaf"+" "+ JSON.parse(localStorage.getItem("userData")).token } }
    );
    console.log("admimdata1--------->",admimdataRes1)
    let admimdataRes=admimdataRes1.data
      if(admimdataRes.status==200){
        this.setState({
          item:{
            hasBlockedMe: false,
            blockedByMe: false,
            uid: admimdataRes.data.adminData._id,
            name: admimdataRes.data.adminData.first_name,
            avatar: `https://admin.illumeetvirtual.com/uploads/${admimdataRes.data.adminData.profile_img}`,
          }
        },()=>{
          this.setChatSidebar()
          console.log("this.state-adminchat module",this.state)
        })
      }
      else{
        this.setChatSidebar()
