...
import renderSelect  from '../common/selectSignupInput';
                      ...
                      >
                        ...
                        </option>
                        {
                          designation.map( data =>
                            <option value={data.title.charAt(0).toUpperCase() + data.title.slice(1)}>{data.title.charAt(0).toUpperCase() + data.title.slice(1)}</option>
                          )
                        }
                      </Field>
...
const validate = values => {
  ...
  if(values.company ){
    ...
    if(!/^([a-zA-Z]+\s)*[a-zA-Z]+$/.test(values.company)){
      errors.company = "Only characters are allowed"
    }

    if(values.company.length>20){
      errors.company = "Name must have length less then 20"
    }
  }
  ...
  if(values.jobTitle ){
    ...
    if(!/^([a-zA-Z]+\s)*[a-zA-Z]+$/.test(values.jobTitle)){
      errors.jobTitle = "Only characters are allowed"
    }

    if(values.jobTitle.length>20){
      errors.jobTitle = "Name must have length less then 20"
    }
  }
  ...
  if(values.role ){
    if(values.role.trim()==null || values.role.trim()==""){
      errors.role = "This field is required";    
    }
  }
  ...
  if(values.password){
    ...
    if (!/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/.test(values.password)) {
      ...
      errors.password = 'Password must contain 1 character and 1 Number and minimum length 8.'
    }
